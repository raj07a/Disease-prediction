...
severity_df = pd.read_csv("Cleaned_Symptom-severity.csv")
...
print(classification_report(y_test, y_pred, labels=np.unique(y),
                            target_names=le.inverse_transform(np.unique(y)), zero_division=0))
...
# ---------------- Step 8: Utility Functions for Streamlit ----------------
